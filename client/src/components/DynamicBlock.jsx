...
const DynamicBlock = () => {
  const { register, control, handleSubmit, watch } = useForm({
    defaultValues: {
      ...
      date: "",
      activities: [
        { sno: 1, activity: "", duration: "", timing: "" }, // Initial activity row
      ],
  ...
  const dummyBlocks = [
    ...
    },
    {
      ...
      paragraph:
        "React uses a component-based architecture where you build encapsulated components that manage their own state, then compose them to make complex UIs. Components can be class-based or functional.",
    },
    {
      sno: 3,
      heading: "Virtual DOM",
      paragraph:
        "React creates a virtual DOM in memory, where it does all the necessary manipulating, before making the changes in the browser DOM. This approach enables the declarative API of React.",
    },
  ];
  ...
  const onSubmit = (data) => {
    ...
    axios.post("http://localhost:3000/generate-doc", data).then((res) => {
      console.log(res);
    });
  };
    ...
    <div className="max-w-4xl mx-auto p-8 bg-gradient-to-br from-indigo-50 to-purple-50 shadow-xl rounded-lg my-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button
          ...
          className="w-full py-3 px-6 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-md shadow-md hover:from-indigo-600 hover:to-purple-600 focus:ring focus:ring-purple-300 transition duration-200 mt-4"
        >
          Add Block
        </button>
        ...
        <button
          type="submit"
          className="w-full py-3 px-6 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-md shadow-md hover:from-emerald-600 hover:to-teal-600 focus:ring focus:ring-emerald-300 transition duration-200 mt-8"
        >
          Submit
        </button>
      </form>
